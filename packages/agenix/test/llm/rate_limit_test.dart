...
DioException _dioError(int status, {Map<String, List<String>> headers = const {}}) {
...
void main() {
  ...
  for (final entry in <(String, (_MockDio, LLM) Function())>[
    ...
    ('Anthropic', () {
      final dio = _MockDio();
      return (dio, Anthropic(apiKey: 'k', modelName: 'claude-sonnet-4-5', client: dio));
    }),
    ('Cohere', () {
      final dio = _MockDio();
      return (dio, Cohere(apiKey: 'k', modelName: 'command-r-plus-08-2024', client: dio));
    }),
  ]) {
    ...
    group(providerName, () {
      test('429 with Retry-After header → LlmRateLimitException with retryAfter set', () async {
        final (dio, llm) = factory();
        when(() => dio.post(any(), data: any(named: 'data'))).thenThrow(
          _dioError(429, headers: {'retry-after': ['30']}),
        );
      ...
      test('429 without Retry-After header → LlmRateLimitException with retryAfter null', () async {
        final (dio, llm) = factory();
        when(() => dio.post(any(), data: any(named: 'data'))).thenThrow(
          _dioError(429),
        );
      ...
      test('non-429 error → LlmException (not LlmRateLimitException) with statusCode set', () async {
        final (dio, llm) = factory();
        when(() => dio.post(any(), data: any(named: 'data'))).thenThrow(
          _dioError(503),
        );
